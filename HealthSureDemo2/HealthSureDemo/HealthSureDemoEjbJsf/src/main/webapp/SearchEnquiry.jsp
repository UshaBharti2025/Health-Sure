...
<body>
    ...
    <div class="container">
     ...
     <!-- Display global validation errors from controller -->
                <h:messages globalOnly="true" layout="table" style="color:red; margin-bottom:10px; font-weight:bold;" />

                  <h:panelGroup layout="block" style="margin-top: 20px;">
                        <h:commandButton value="Show all Recipients"
                        action="#{recipientController.show}"
                        styleClass="btn" />
        ...
        <div class="form-wrapper">
            ...
            <!-- FOR SEARCH TYPE FIELD -->
                <div class="form-group">
                    ...
                    <h:selectOneMenu id="searchType" value="#{recipientController.searchType}" onchange="submit()">
                       <f:selectItem itemValue="" itemLabel="--Select--" />
                        ...
                        <f:selectItem itemLabel="createdAt" itemValue="Created Date" />
                   </h:selectOneMenu>
                         <h:message for="searchType" styleClass="error-msg" />
                 </div>
                ...
                <!--INPUT VALUE -->
                <!--Only show when searchType is NOT 'createdAt' -->
                <h:panelGroup id="valueField" rendered="#{recipientController.searchType ne 'createdAt'}">
                <div class="form-group">
                     <h:outputLabel for="searchValue" value="Enter Value:" />
                      <!-- CASE: Mobile Number (only digits, max 10) -->
        

        <!-- CASE: All other search types -->
        <h:panelGroup rendered="#{recipientController.searchType ne 'mobile'}">
            <h:inputText id="searchValueGeneric" value="#{recipientController.searchValue}" />
            <h:message for="searchValueGeneric" styleClass="error-msg" />
        </h:panelGroup>
        <h:panelGroup rendered="#{recipientController.searchType eq 'mobile'}">
            <h:inputText id="searchValueMobile" value="#{recipientController.searchValue}" maxlength="10">
                <f:validateRegex pattern="\d{1,10}" />
            </h:inputText>
            <h:message for="searchValueMobile" styleClass="error-msg" />
        </h:panelGroup>
                ...
                <h:panelGroup id="searchModeBlock">
...
<!-- Visible only when searchType IS 'createdAt' -->
<h:panelGroup id="dateFields" rendered="#{recipientController.searchType eq 'createdAt'}">
    <div class="form-group">
...
<div class="table-wrapper">
    <h:form id="tableForm">
        ...
        <h:dataTable value="#{recipientController.resultList}" var="r"
                    rendered="#{not empty recipientController.resultList}"
                    styleClass="data-table" border="1">
            
            <!-- Health ID Column -->
            <h:column>
                <f:facet name="header"><h:outputText value="Health ID"/></f:facet>
                <h:commandLink value="#{r.hId.toUpperCase()}" action="#{recipientController.prepareUpdate}">
